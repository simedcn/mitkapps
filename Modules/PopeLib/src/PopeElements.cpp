...
Elements::Elements()
{
	/*vector<uint> temp_uint;
	get_mean(temp_uint);
	get_std_dev(temp_uint);
	vector<float> temp_float;
	get_mean(temp_float);
	get_std_dev(temp_float);*/
}
...
QString Elements::get_patientId_or_patientName(const QString& filename, bool show_warning, const QString& def_value)
{
	...
	string name = filename.toLocal8Bit().constData();
	//QTextCodec *codec = QTextCodec::codecForName("UTF-8");
	//QByteArray encodedString = codec->fromUnicode(filename);
	//string name = encodedString;
...
QString Elements::get_short_name_for_image(const string& name, const size_t max_length)
{
...
bool Elements::split_property(const std::string& str_prop, std::vector<float>* values)
{
	bool is_ok = true;
	if (values == nullptr)
		return false;
	values->clear();
	constexpr size_t NA = numeric_limits<size_t>::max();
	size_t i_start = NA;
	//bool is_point = false;
	for (size_t i = 0; i < str_prop.length(); i++)
	{
		size_t i_end = NA;
		const auto& ch = str_prop[i];
		...
		if (i_end == NA && i == str_prop.length() - 1)
		{
			i_end = i + 1;
		}
		if (i_start != NA && i_end != NA)
		{
			try
			{
				assert(i_end > i_start);
				string str_val = str_prop.substr(i_start, i_end - i_start);
...
/*template<typename T>
float Elements::get_mean(const std::vector<T>& v)
{
	T sum = std::accumulate(v.begin(), v.end(), 0.0f);
	float mean = float(sum) / v.size();
	return mean;
}*/
float Elements::get_mean(const std::vector<uint>& v)
//float Elements::get_mean(const std::vector<uint>& v)
{
...
float Elements::get_mean(const std::vector<float>& v)
{
...
/*template<typename T>
float Elements::get_std_dev(const std::vector<T>& v, float* mean_value)
{
	float mean = (mean_value == nullptr) ? get_mean(v) : *mean_value;
...
float Elements::get_std_dev(const std::vector<uint>& v, float* mean_value)
{
	float mean = (mean_value == nullptr) ? get_mean(v) : *mean_value;
	std::vector<float> diff(v.size());
	std::transform(v.begin(), v.end(), diff.begin(), [mean](float x) { return x - mean; });
...
float Elements::get_std_dev(const std::vector<float>& v, float* mean_value)
{
	float mean = (mean_value == nullptr) ? get_mean(v) : *mean_value;
	std::vector<float> diff(v.size());
	std::transform(v.begin(), v.end(), diff.begin(), [mean](float x) { return x - mean; });
