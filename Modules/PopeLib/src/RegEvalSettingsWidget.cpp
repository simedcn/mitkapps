...
#include "RegEvalSettingsWidget.h"
...
void RegEvalSettingsWidget::SetNode(mitk::DataNode* node)
//void RegEvalSettingsWidget::SetNode(mitk::DataNode::Pointer node)
{
...
RegEvalSettingsWidget::RegEvalSettingsWidget(QWidget *parent)
...
void RegEvalSettingsWidget::ConfigureControls()
{
	...
	this->slideBlend->installEventFilter(this);
	m_internalUpdate = false;
}

bool RegEvalSettingsWidget::eventFilter(QObject* object, QEvent* event)
{
	if (object == this->slideBlend && this->slideBlend->isEnabled() && event->type() == QEvent::MouseButtonDblClick)
	{
		this->sbBlend->setValue(50);
		event->accept();
		return true;
	}
...
void RegEvalSettingsWidget::OnComboStyleChanged(int index)
{
...
void RegEvalSettingsWidget::OnBlend50Pushed()
{
	sbBlend->setValue(50);
};

void RegEvalSettingsWidget::OnBlendTargetPushed()
{
	sbBlend->setValue(0);
};

void RegEvalSettingsWidget::OnBlendMovingPushed()
{
	sbBlend->setValue(100);
};

void RegEvalSettingsWidget::OnBlendTogglePushed()
{
	sbBlend->setValue(100 - sbBlend->value());
};

void RegEvalSettingsWidget::OnSlideBlendChanged(int factor)
{
...
void RegEvalSettingsWidget::OnSpinBlendChanged(int factor)
{
	if (m_selectedEvalNode.IsNotNull())
	{
...
void RegEvalSettingsWidget::OnSpinCheckerChanged(int count)
{
	if (m_selectedEvalNode.IsNotNull())
	{
...
void RegEvalSettingsWidget::OnWipeStyleChanged()
{
	if (m_selectedEvalNode.IsNotNull())
	{
...
void RegEvalSettingsWidget::OnContourStyleChanged()
{
	if (m_selectedEvalNode.IsNotNull())
	{
