...
void ManualRegistration::CreateQtPartControl(QWidget* parent)
{
	...
	QString str = QString::fromLatin1("on_mouse_moved");
	m_MouseMoveSupplier->RegisterForUpdates(this, str);
...
void ManualRegistration::on_mouse_moved(QVector<double> point)
{
	...
	if (point.size() < 3)
		return;
	//const double dbl_min = 6 * DBL_EPSILON; // std::numeric_limits<double>::epsilon()
...
void ManualRegistration::OnStoreBtnPushed()
{
	...
	if (ui.checkMapEntity->isChecked())
	{
		...
		double min_val = -1024; // 100
		double max_val = 3071; // 200
		try
		{
			const mitk::Image* image = dynamic_cast<const mitk::Image*>(pMapJob->m_spInputData.GetPointer());
			if (image)
			{
				auto statistics = image->GetStatistics();
				min_val = statistics->GetScalarValueMin();
				max_val = statistics->GetScalarValueMax();
			}
		}
		catch (...)
		{}
		...
		pMapJob->m_paddingValue = min_val;
		pMapJob->m_allowUnregPixels = true;
		pMapJob->m_errorValue = max_val;
...
void ManualRegistration::OnMapResultIsAvailable(mitk::BaseData::Pointer spMappedData, const QmitkMappingJob* job)
{
	...
	auto original_properties = job->m_spInputData->GetPropertyList()/*->Clone()*/;
