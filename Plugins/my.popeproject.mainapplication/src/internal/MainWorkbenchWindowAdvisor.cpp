...
MainWorkbenchWindowAdvisor::MainWorkbenchWindowAdvisor(berry::WorkbenchAdvisor* wbAdvisor, berry::IWorkbenchWindowConfigurer::Pointer configurer)
	: QmitkExtWorkbenchWindowAdvisor(wbAdvisor, configurer)
{}
MainWorkbenchWindowAdvisor::~MainWorkbenchWindowAdvisor()
{}

void MainWorkbenchWindowAdvisor::setPluginTitle(const Elements::PluginDescriptor& plugin, berry::IWorkbenchPage::Pointer page)
{
	if (page == nullptr)
	{
...
ViewDescriptors MainWorkbenchWindowAdvisor::createViewDescriptors()
 {
	 ...
	 for (const auto& viewDescriptor : viewDescriptors)
	 {
		 ...
		 // If viewExcludeList is set, it contains the id-strings of view, which should not appear as an menu-entry in the menu
...
void MainWorkbenchWindowAdvisor::manageViews(berry::IWorkbenchPage::Pointer page, berry::IPreferences::Pointer mainApplicationPreferencesNode, bool toConfigureTitles)
{
	/// Open/close plugins.
	if (page == nullptr)
		return;
...
void MainWorkbenchWindowAdvisor::maximizeWindow_IfNotOverridden(berry::IWorkbenchWindow::Pointer window)
{
	if (window == nullptr)
		return;
...
void MainWorkbenchWindowAdvisor::PostWindowCreate()
{
	...
	auto mainApplicationPreferencesNode = systemPrefs->Node("/my.popeproject.mainapplication");
	manageViews(page, mainApplicationPreferencesNode, false);
	...
	//std::string str2 = perspAction->icon().name().toStdString();*/
}
bool MainWorkbenchWindowAdvisor::PreWindowShellClose()
{
	...
	{
		berry::IPreferencesService* prefService = berry::Platform::GetPreferencesService();
		auto mainApplicationPreferencesNode = prefService->GetSystemPreferences()->Node("/my.popeproject.mainapplication");
...
void MainWorkbenchWindowAdvisor::OnPreferencesChanged(const berry::IBerryPreferences* preferences)
{
	...
	if (prefs == nullptr)
		return;
	//string str = prefs->Name().toStdString();      == "my.popeproject.mainapplication"
	...
	auto systemPrefs = prefService->GetSystemPreferences();
	auto mainApplicationPreferencesNode = systemPrefs->Node("/my.popeproject.mainapplication");
	manageViews(page, mainApplicationPreferencesNode);
}
