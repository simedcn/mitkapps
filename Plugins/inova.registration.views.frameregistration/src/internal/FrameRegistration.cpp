...
FrameRegistration::FrameRegistration()
	: m_Parent(nullptr), m_LoadedDLLHandle(nullptr)
	, m_LoadedAlgorithm(nullptr), m_CanLoadAlgorithm(false)
	, m_ValidInputs(false)
	, m_Working(false)
{
...
void FrameRegistration::OnSelectedAlgorithmChanged()
{
...
void FrameRegistration::OnLoadAlgorithmButtonPushed()
{
...
void FrameRegistration::CreateQtPartControl(QWidget* parent)
{
	...
	m_AlgorithmSelectionListener.reset(new berry::SelectionChangedAdapter<FrameRegistration>(this, &FrameRegistration::OnAlgorithmSelectionChanged));
...
bool FrameRegistration::CheckInputs()
{
	...
	{
		m_Controls.m_lbLoadedAlgorithmName->setText(QString("<font color='red'>No algorithm seleted!</font>"));
	...
	else
	{
		...
		else
		{
			...
			if (targetImage)
			{
				...
				{
					m_Controls.m_lbTargetName->setText(QString("<font color='red'>wrong image dimension. ") + QString::number(m_LoadedAlgorithm->getTargetDimensions()) + QString("D+t needed</font>"));
				}
				else
				{
					validTarget = true;
...
std::string FrameRegistration::GetInputNodeDisplayName(const mitk::DataNode* node)
const
{
...
void FrameRegistration::OnSelectionChanged(berry::IWorkbenchPart::Pointer,
...
{
	m_Controls.m_teLog->append(QString("<b><font color='blue'>Corrected image stored. Name: ") + QString::fromStdString(job->m_MappedName) + QString("</font></b>"));
...
void FrameRegistration::OnAlgorithmSelectionChanged(const berry::IWorkbenchPart::Pointer& sourcepart, const berry::ISelection::ConstPointer& selection)
{
...
void FrameRegistration::UpdateAlgorithmSelection(berry::ISelection::ConstPointer selection)
{
