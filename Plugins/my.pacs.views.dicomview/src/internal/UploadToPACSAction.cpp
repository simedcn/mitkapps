...
void UploadToPACSAction::Run(const QList<DataNode::Pointer> &selectedNodes)
{
  ...
  QString host = dicomViewPreferencesNode->Get("PACS IP", "127.0.0.1");
  int port = dicomViewPreferencesNode->GetInt("PACS port", 11112);
  QString storageAETitle = dicomViewPreferencesNode->Get("PACS storage AETitle", "SERVERAE");
  QString AETitle = dicomViewPreferencesNode->Get("PACS AETitle", "SERVERAE");
  QStringList storescuArgs;
  storescuArgs << "-aec" << AETitle;
  storescuArgs << "-aet" << storageAETitle;
  storescuArgs << host;
