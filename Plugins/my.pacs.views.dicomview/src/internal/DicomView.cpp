...
void DicomView::addSeriesToDataManager(const QStringList& listOfFilesForSeries, const shared_ptr<QString> modality)
{
    if (!listOfFilesForSeries.isEmpty())
    {
                ...
                if (!readerOutput.empty())
                {
                    ...
                    if (doseImage != nullptr)
                    {
                        ...
                        if (sopUIDProperty.IsNotNull())
                        {
                            ...
                            {
                                string sopUID = sopUIDStringProperty->GetValue();
        ...
        else
        {
            ...
            /*
            if (!seriesToLoad.empty() && DicomSeriesReader::IsPhilips3DDicom(seriesToLoad.front()))
            {
                MITK_INFO << "it is a Philips3D US Dicom file" << endl;
                ...
                DicomSeriesReader::StringContainer stringvec;
            ...
            for (unsigned int i = 0; i < reader->GetNumberOfOutputs(); ++i)
            {
                ...
                string nodeName = "Unnamed_DICOM";

                string studyDescription = desc.GetPropertyAsString("studyDescription");
                string seriesDescription = desc.GetPropertyAsString("seriesDescription");
            ...
            {
                MITK_ERROR << "Error loading series " << ((modality == nullptr) ? "" : *modality)
                           << " (" << listOfFilesForSeries.size() << " series in total)";
            }
        }
    }
    else
