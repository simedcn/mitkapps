...
void LocalSCU::notifyInstanceStored(const OFString &, const OFString &, const OFString &) const
{
...
void DicomView::CreateQtPartControl(QWidget *parent)
{
    ...
    //auto context = my_pacs_views_dicomview_Activator::GetPluginContext();
...
void DicomView::on_pushButton_Retrieve_clicked()
{
    ...
    for (const QString& studyUID : selectedStudiesUIDs)
    {
        ...
        if (protocol == PROTOCOL_CGET) // (query->preferCGET()) //(ui.radioButton_Protocol_CGET->isChecked())
        {
            ...
            auto lambda = [this, progressLabel](QString value) {
                on_retrieveProgress_changed(progressLabel, value);
            };
...
void DicomView::on_lineEdit_StorageAETitle_textChanged(QString)
{
...
void DicomView::on_table_Series_selectionChanged(const QStringList& )
{
    updateButtons();
}
