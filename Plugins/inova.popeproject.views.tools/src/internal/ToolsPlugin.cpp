...
void ToolsPlugin::updateAfterSelectionChanged()
{
    ...
    if (is_selected_node)
    {
        ...
        else
        {
            ...
            const uint max_length = 28 + str_center_replacement.length();
...
void ToolsPlugin::updateStatistics(const StatData& statistics, mitk::Image* image)
{
    ...
    for (unsigned int t = 0; t < num_timeSteps; t++)
    {
        ...
        for (uint i = 0; i < percetiles.size(); i++)
        {
        ...
        for (uint i = 0; i < percetiles.size() - 1; i++)
        {
...
void ToolsPlugin::NodeAdded(const mitk::DataNode*)
{
...
void ToolsPlugin::on_checkBox_GroupTags_toggled(bool)
{
