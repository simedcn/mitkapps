...
void detect_beginning(vector<double> diff, const double& min_threshold, const double& max_threshold, int* i_beginning, int* i_end)
{
	...
	size_t i1_1 = 0;
	for (size_t i = i1_1; i < diff.size(); i++)
	{
		i1_1 = i;
	...
	// End of start region
	size_t i1_2 = i1_1 + 1;
	for (size_t i = i1_2; i < diff.size(); i++)
	{
		i1_2 = i;
...
void ToolsPlugin::updateAfterSelectionChanged()
{
	...
	if (is_selected_node)
	{
		...
		else
		{
			...
			const size_t max_length = 28 + str_center_replacement.length();
			if (name.length() > max_length)
			{
				stringstream ss;
				const size_t half = (max_length - str_center_replacement.length()) / 2;
...
void ToolsPlugin::updateStatistics(const StatData& statistics, mitk::Image* image)
{
	...
	for (unsigned int t = 0; t < num_timeSteps; t++)
	{
		...
		for (size_t i = 0; i < percetiles.size(); i++)
		{
		...
		for (size_t i = 0; i < percetiles.size() - 1; i++)
		{
	...
	{// dataNode == nullptr when nothing is selected or the selection doesn't contain an image.
	 ...
	 /// Get the settings node.
		...
		if (has_many_images)
		{
			...
			bool def_val = m_ToolsPluginPreferencesNode->GetBool("3D autorotation", false);
...
void ToolsPlugin::on_ThreadedStatisticsCalculation_finished()
{
	...
	for (int t = 0; t < num_t; t++)
	{
		...
		assert(dimemsions_size.GetSize() == 1);
