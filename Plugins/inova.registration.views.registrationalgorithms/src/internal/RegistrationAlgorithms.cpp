...
void RegistrationAlgorithms::CreateConnections()
{
	...
	connect(m_Controls.checkMovingMask, SIGNAL(toggled(bool)), this, SLOT(OnMaskCheckBoxToggeled(bool)));
	connect(m_Controls.checkTargetMask, SIGNAL(toggled(bool)), this, SLOT(OnMaskCheckBoxToggeled(bool)));
...
void RegistrationAlgorithms::OnSelectedAlgorithmChanged()
{
	...
	m_Controls.m_lbSelectedAlgorithm->setText(QString::fromStdString(currentItemInfo->getAlgorithmUID().getName()));
...
void RegistrationAlgorithms::OnLoadAlgorithmButtonPushed()
{
	...
	::map::deployment::DLLHandle::Pointer tempDLLHandle = ::map::deployment::openDeploymentDLL(dllInfo->getLibraryFilePath());
	::map::algorithm::RegistrationAlgorithmBase::Pointer tempAlgorithm = ::map::deployment::getRegistrationAlgorithm(tempDLLHandle);
...
void RegistrationAlgorithms::CreateQtPartControl(QWidget* parent)
{
	...
	m_AlgorithmSelectionListener.reset(new berry::SelectionChangedAdapter<RegistrationAlgorithms>(this, &RegistrationAlgorithms::OnAlgorithmSelectionChanged));

	// register selection listener
	GetSite()->GetWorkbenchWindow()->GetSelectionService()->AddSelectionListener(m_AlgorithmSelectionListener.data());
	...
	berry::ISelection::ConstPointer selection = GetSite()->GetWorkbenchWindow()->GetSelectionService()->GetSelection("org.mitk.views.matchpoint.algorithm.browser");
...
bool RegistrationAlgorithms::CheckInputs()
{
		...
		else
		{
			...
			else if (movingImage && !pPointSetInterface)
			{
				if (movingImage->GetDimension() - 1 == m_LoadedAlgorithm->getMovingDimensions() && movingImage->GetTimeSteps() > 1)
				{
					...
					validMoving = true;
					m_Controls.m_teLog->append(QString("<font color='gray'><i>Selected moving image has multiple time steps. First time step is used as moving image.</i></font>"));
				}
				else if (movingImage->GetDimension() != m_LoadedAlgorithm->getMovingDimensions())
				{
					m_Controls.m_lbMovingName->setText(QString("<font color='red'>wrong image dimension. ") + QString::number(m_LoadedAlgorithm->getMovingDimensions()) + QString("D needed</font>"));
				}
				else
				{
					validMoving = true;
		...
		else
		{
			...
			else if (targetImage && !pPointSetInterface)
			{
				if (targetImage->GetDimension() - 1 == m_LoadedAlgorithm->getTargetDimensions() && targetImage->GetTimeSteps() > 1)
				{
					...
					validTarget = true;
					m_Controls.m_teLog->append(QString("<font color='gray'><i>Selected target image has multiple time steps. First time step is used as target image.</i></font>"));
				}
				else if (targetImage->GetDimension() != m_LoadedAlgorithm->getTargetDimensions())
				{
					m_Controls.m_lbTargetName->setText(QString("<font color='red'>wrong image dimension. ") + QString::number(m_LoadedAlgorithm->getTargetDimensions()) + QString("D needed</font>"));
				}
				else
				{
					validTarget = true;
			...
			else
			{
				...
				else if (movingMaskImage && pMaskInterface)
				{
					if (movingMaskImage->GetDimension() - 1 == m_LoadedAlgorithm->getMovingDimensions() && movingMaskImage->GetTimeSteps() > 1)
					{
						...
						m_Controls.m_teLog->append(QString("<font color='gray'><i>Selected moving mask has multiple time steps. First time step is used as moving mask.</i></font>"));
					}
					else if (movingMaskImage->GetDimension() != m_LoadedAlgorithm->getMovingDimensions())
					{
						m_Controls.m_lbMovingMaskName->setText(QString("<font color='red'>wrong image dimension. ") + QString::number(m_LoadedAlgorithm->getMovingDimensions()) + QString("D needed</font>"));
					}
					else
					{
				...
				{
					m_Controls.m_lbMovingMaskName->setText(QString("<font color='red'>no supported data selected!</font>"));
				}
			...
			else
			{
				...
				else if (targetMaskImage && pMaskInterface)
				{
					if (targetMaskImage->GetDimension() - 1 == m_LoadedAlgorithm->getTargetDimensions() && targetMaskImage->GetTimeSteps() > 1)
					{
						...
						m_Controls.m_teLog->append(QString("<font color='gray'><i>Selected target mask has multiple time steps. First time step is used as target mask.</i></font>"));
					}
					else if (targetMaskImage->GetDimension() != m_LoadedAlgorithm->getTargetDimensions())
					{
						m_Controls.m_lbTargetMaskName->setText(QString("<font color='red'>wrong image dimension. ") + QString::number(m_LoadedAlgorithm->getTargetDimensions()) + QString("D needed</font>"));
					}
					else
					{
				...
				{
					m_Controls.m_lbTargetMaskName->setText(QString("<font color='red'>no supported data selected!</font>"));
				}
			}
	...
	if (validMoving)
	{
		m_Controls.m_lbMovingName->setText(QString::fromStdString(GetInputNodeDisplayName(m_spSelectedMovingNode)));
	}

	if (validTarget)
	{
		m_Controls.m_lbTargetName->setText(QString::fromStdString(GetInputNodeDisplayName(m_spSelectedTargetNode)));
	}

	if (validMovingMask && m_Controls.checkMovingMask->isChecked())
	{
		m_Controls.m_lbMovingMaskName->setText(QString::fromStdString(GetInputNodeDisplayName(m_spSelectedMovingMaskNode)));
	}

	if (validTargetMask && m_Controls.checkTargetMask->isChecked())
	{
		m_Controls.m_lbTargetMaskName->setText(QString::fromStdString(GetInputNodeDisplayName(m_spSelectedTargetMaskNode)));
	}
...
std::string RegistrationAlgorithms::GetInputNodeDisplayName(const mitk::DataNode* node) const
{
...
void RegistrationAlgorithms::ConfigureRegistrationControls()
{
	...
	if (m_LoadedAlgorithm.IsNotNull())
	{
		...
		this->m_Controls.m_lbLoadedAlgorithmName->setText(QString::fromStdString(m_LoadedAlgorithm->getUID()->toStr()));
	}
	else
	{
		m_Controls.m_tabSettings->setEnabled(false);
		m_Controls.m_tabExecution->setEnabled(false);
		this->m_Controls.m_lbLoadedAlgorithmName->setText(QString("<font color='red'>no algorithm loaded!</font>"));
...
void RegistrationAlgorithms::OnSelectionChanged(berry::IWorkbenchPart::Pointer,
...
void RegistrationAlgorithms::OnRegResultIsAvailable(mitk::MAPRegistrationWrapper::Pointer
		spResultRegistration, const QmitkRegistrationJob* pRegJob)
{
	mitk::DataNode::Pointer spResultRegistrationNode = mitk::generateRegistrationResultNode(pRegJob->m_JobName, spResultRegistration, pRegJob->GetLoadedAlgorithm()->getUID()->toStr(), pRegJob->m_MovingDataUID, pRegJob->m_TargetDataUID);

	if (pRegJob->m_StoreReg)
	{
		m_Controls.m_teLog->append(QString("<b><font color='blue'> Storing registration object in data manager ... </font></b>"));
	...
	if (m_Controls.m_checkMapEntity->checkState() == Qt::Checked)
	{
		...
		m_Controls.m_teLog->append(QString("<b><font color='blue'>Started mapping input data...</font></b>"));
...
void RegistrationAlgorithms::OnMapResultIsAvailable(mitk::BaseData::Pointer spMappedData,
...
void RegistrationAlgorithms::OnAlgorithmIterated(QString info, bool hasIterationCount,
...
void RegistrationAlgorithms::OnAlgorithmSelectionChanged(const berry::IWorkbenchPart::Pointer& sourcepart, const berry::ISelection::ConstPointer& selection)
{
...
void RegistrationAlgorithms::UpdateAlgorithmSelection(berry::ISelection::ConstPointer selection)
{
	mitk::MAPAlgorithmInfoSelection::ConstPointer currentSelection = selection.Cast<const mitk::MAPAlgorithmInfoSelection>();

	if (currentSelection)
	{
		mitk::MAPAlgorithmInfoSelection::AlgorithmInfoVectorType infoVector = currentSelection->GetSelectedAlgorithmInfo();
