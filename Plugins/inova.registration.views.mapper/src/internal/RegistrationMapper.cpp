...
const std::string RegistrationMapper::VIEW_ID = "inova.registration.views.mapper";
...
void RegistrationMapper::SetFocus()
{
	//m_Controls.buttonPerformImageProcessing->setFocus();
}

void RegistrationMapper::CreateConnections()
{
	// show first page
...
void RegistrationMapper::Error(QString msg)
{
...
void RegistrationMapper::CreateQtPartControl(QWidget* parent)
{
...
bool RegistrationMapper::IsAbleToRefineGeometry() const
{
	bool result = false;

	if (this->m_spSelectedRegNode.IsNotNull())
	{
		const mitk::MAPRegistrationWrapper* wrapper = dynamic_cast<const mitk::MAPRegistrationWrapper*>(this->m_spSelectedRegNode->GetData());
...
bool RegistrationMapper::IsBinaryInput() const
{
...
bool RegistrationMapper::IsPointSetInput() const
{
	bool result = false;
...
mitk::DataNode::Pointer RegistrationMapper::GetSelectedRegNode()
{
...
QList<mitk::DataNode::Pointer> RegistrationMapper::GetSelectedDataNodes()
{
	typedef QList<mitk::DataNode::Pointer> NodeListType;
...
mitk::DataNode::Pointer RegistrationMapper::GetAutoRefNodeByReg()
{
	mitk::DataNode::Pointer spResult = nullptr;
	...
	if (spResult.IsNull() && this->m_spSelectedInputNode.IsNotNull())
	{
		...
		m_Controls.m_teLog->append(QString("<font color='gray'><i>Cannot determine reference automatically. Use input image as reference.</i></font>"));
	}

	return spResult;
}

void RegistrationMapper::CheckInputs()
{
	...
	else
	{
		...
		{
			m_Controls.m_lbRegistrationName->setText(QString("<font color='red'>") + QString::fromStdString(this->m_spSelectedRegNode->GetName()) + QString("</font>"));
		}
	}
	...
	else
	{
		...
		{
			m_Controls.m_lbInputName->setText(QString("<font color='red'>") + QString::fromStdString(this->m_spSelectedInputNode->GetName()) + QString("</font>"));
		}
	...
	else
	{
		if (this->m_ValidRef)
		{
			...
			if (this->m_spSelectedRefNode->GetData() && this->m_spSelectedRefNode->GetData()->GetTimeSteps() > 1)
			{
				m_Controls.m_teLog->append(QString("<font color='gray'><i>Selected reference image has multiple time steps. Only geometry of time step 1 is used as reference.</i></font>"));
			}
		}
		else
		{
			m_Controls.m_lbReferenceName->setText(QString("<font color='red'>") + QString::fromStdString(this->m_spSelectedRefNode->GetName()) + QString("</font>"));
		}
	}
...
void RegistrationMapper::CheckNodesValidity(bool& validReg, bool& validInput, bool& validRef)
{
	...
	if (this->m_spSelectedRegNode.IsNotNull())
	{
		if (this->m_spSelectedInputNode.IsNotNull())
		{
			...
			const mitk::MAPRegistrationWrapper* wrapper = dynamic_cast<const mitk::MAPRegistrationWrapper*>(m_spSelectedRegNode->GetData());
			mitk::Image* inputImage = dynamic_cast<mitk::Image*>(m_spSelectedInputNode->GetData());

			if (inputImage && wrapper)
			{
				if (inputImage->GetDimension() - 1 == wrapper->GetMovingDimensions() && inputImage->GetTimeSteps() > 1)
				{
...
void RegistrationMapper::ConfigureMappingControls()
{
	this->m_Controls.m_pbMap->setEnabled(this->m_ValidInput  && this->m_ValidRef && (this->m_ValidReg || this->m_spSelectedRegNode.IsNull()));
	this->m_Controls.m_pbRefine->setEnabled(this->m_ValidInput && this->m_ValidReg && this->IsAbleToRefineGeometry() && !this->IsPointSetInput());
	...
	{
		this->m_Controls.m_leMappedName->setText(tr("mapped_by_") + QString::fromStdString(m_spSelectedRegNode->GetName()));
	}
	else
	{
	...
	if (this->IsBinaryInput() != this->m_preparedForBinaryInput)
	{
		...
		{
			m_Controls.m_teLog->append(QString("<font color='gray'><i>Binary input (mask) detected. Preparing for mask mapping (default interpolation: nearest neigbour; padding value: 0)</i></font>"));
...
void RegistrationMapper::ConfigureProgressInfos()
{

}

void RegistrationMapper::OnSelectionChanged(berry::IWorkbenchPart::Pointer, const QList<mitk::DataNode::Pointer>&)
{
	this->CheckInputs();
	this->ConfigureMappingControls();
}

void RegistrationMapper::OnLockRegButtonPushed()
{
...
void RegistrationMapper::OnLockInputButtonPushed()
{
...
void RegistrationMapper::OnLockReferenceButtonPushed()
{
...
void RegistrationMapper::OnManualRefChecked()
{
	this->CheckInputs();
	this->ConfigureMappingControls();
}

void RegistrationMapper::OnLinkSampleFactorChecked()
{
...
void RegistrationMapper::OnMapBtnPushed()
{
	SpawnMappingJob();
}

void RegistrationMapper::OnRefineBtnPushed()
{
	SpawnMappingJob(true);
}

void RegistrationMapper::SpawnMappingJob(bool doGeometryRefinement)
{
	...
	if (m_spSelectedRegNode.IsNull())
	{
		...
		m_Controls.m_teLog->append(QString("<font color='gray'><i>No registration selected. Preforming mapping with identity transform</i></font>"));
	}
	...
	connect(pJob, SIGNAL(MapResultIsAvailable(mitk::BaseData::Pointer, const QmitkMappingJob*)), this, SLOT(OnMapResultIsAvailable(mitk::BaseData::Pointer, const QmitkMappingJob*)), Qt::BlockingQueuedConnection);
	connect(pJob, SIGNAL(AlgorithmInfo(QString)), this, SLOT(OnMappingInfo(QString)));

	m_Controls.m_teLog->append(QString("<b><font color='blue'>Started mapping job. Name: ") + m_Controls.m_leMappedName->text() + QString("</font></b>"));
...
void RegistrationMapper::OnMapJobError(QString err)
{
	Error(err);
};

void RegistrationMapper::OnMapResultIsAvailable(mitk::BaseData::Pointer spMappedData, const QmitkMappingJob* job)
{
...
void RegistrationMapper::OnMappingInfo(QString info)
{
	m_Controls.m_teLog->append(QString("<font color='gray'><i>") + info + QString("</i></font>"));
};

void RegistrationMapper::OnXFactorChanged(double d)
{
	if (m_Controls.m_cbLinkFactors->isChecked())
	{
